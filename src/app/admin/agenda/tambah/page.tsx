...
import { createAgenda } from "@/src/app/actions/agenda"; 
...
export default function TambahAgendaPage() {
  ...
  if (!isLoaded) return <div className="p-10 text-center text-gray-500">Memuat data pengguna...</div>;
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Isi formulir di bawah untuk mempublikasikan kegiatan.</p>
        </div>
      </div>

      <form action={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8 space-y-6">
        
        {/* --- 1. UPLOAD THUMBNAIL (SUPABASE) --- */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Poster / Thumbnail Kegiatan</label>
          ...
          {!thumbnailUrl && (
            <p className="text-xs text-amber-600 mt-2">* Gambar opsional, tapi disarankan agar tampilan menarik.</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-1">Judul Agenda <span className="text-red-500">*</span></label>
            <input 
              name="judul" 
              required 
              type="text" 
          ...
          <div>
            ...
            <div className="relative">
              <select name="jenis" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 bg-white outline-none">
        ...
        <div className="border-t border-gray-100 pt-4">
          <h3 className="text-sm font-bold text-gray-900 mb-4 uppercase tracking-wider">Waktu & Tempat</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai <span className="text-red-500">*</span></label>
              <input name="tanggalMulai" required type="date" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 outline-none" />
            </div>
            
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai <span className="text-xs text-gray-400">(Opsional)</span></label>
              <input name="tanggalSelesai" type="date" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 outline-none" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Jam Mulai</label>
                <input name="waktuMulai" type="time" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Jam Selesai</label>
                <input name="waktuSelesai" type="time" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 outline-none" />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Lokasi Kegiatan <span className="text-red-500">*</span></label>
              <input 
                name="lokasi" 
                required 
                type="text" 
